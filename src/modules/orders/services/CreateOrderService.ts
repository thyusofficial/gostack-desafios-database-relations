...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (checkInexistentProducts.length) {
      throw new AppError(`Product ${checkInexistentProducts[0]} not found`);
    }
    ...
    if (findProductsWithUnavailableQuantity.length) {
      throw new AppError(
        `The quantity ${findProductsWithUnavailableQuantity[0].quantity} is not available for ${findProductsWithUnavailableQuantity[0].id}`,
      );
    }

    const serialieProducts = products.map(product => ({
